...
import es.cvmaker.upv.dropbox.Actividad_dropbox;
...
public class MuPDFActivity extends Activity {
	...
	// CVMaker
	private final String TAG_APLICACION = "CVMaker";
	String rutaFichero;
...
// XXX	private ImageButton  mLinkButton;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (core == null) {
			...
			if (Intent.ACTION_VIEW.equals(intent.getAction())) {
				Uri uriRutaFichero = intent.getData();
				if (uriRutaFichero.toString().startsWith(
						"content://media/external/file")) {
					Cursor cursor = getContentResolver().query(uriRutaFichero,
							new String[] { "_data" }, null, null, null);
					if (cursor.moveToFirst()) {
						uriRutaFichero = Uri.parse(cursor.getString(0));
					}
				}
				rutaFichero = Uri.decode(uriRutaFichero.getEncodedPath());
				core = openFile(rutaFichero);
			} else {
				try {
					rutaFichero = getIntent().getExtras().getString("ruta");
					core = new MuPDFCore(rutaFichero);
				} catch (Exception e) {
					Toast.makeText(this, "No se puede abrir el documento",
		...
		if (core == null)
		{
			...
			alert.setButton(AlertDialog.BUTTON_POSITIVE, getString(R.string.txtAceptar),
	...
	public void compartirPDF(View v) {
		Intent intent = new Intent(Intent.ACTION_SEND);
		intent.setType("application/pdf");

		try {
			File ficheroPDF = new File(rutaFichero);
			Uri u = Uri.fromFile(ficheroPDF);
			intent.putExtra(Intent.EXTRA_STREAM, u);
			startActivity(Intent.createChooser(intent,
					getString(R.string.sharevia)));
		} catch (Exception e) {
			Toast.makeText(this, "ERROR", Toast.LENGTH_SHORT).show();
		}
	}

	public void subirDropbox(View v) {
		Intent i = new Intent(this, Actividad_dropbox.class);
		i.putExtra("pathFile", rutaFichero);
		startActivity(i);
	}
...
// XXX							linkPage = pageView.hitLinkPage(e.getX(), e.getY());
		...
		mFilenameView.setText(TAG_APLICACION);
